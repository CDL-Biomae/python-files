...
def datesClesFusion(id_mp_alim, id_mp_chimie, id_mp_repro):
    exposureconditions_alim = QueryScript(
        script='SELECT step, recordedAt, barrel FROM measureexposurecondition WHERE measurepoint_id=' + str(
            id_mp_alim)).execute()
    exposureconditions_chimie = QueryScript(
        script='SELECT step, recordedAt, barrel FROM measureexposurecondition WHERE measurepoint_id=' + str(
            id_mp_chimie)).execute()
    exposureconditions_repro = QueryScript(
        script='SELECT step, recordedAt, barrel FROM measureexposurecondition WHERE measurepoint_id=' + str(
            id_mp_repro)).execute()

    steps_barrels_alim = [(x[0], x[2]) for x in exposureconditions_alim]
    steps_barrels_chimie = [(x[0], x[2]) for x in exposureconditions_chimie]
    steps_barrels_repro = [(x[0], x[2]) for x in exposureconditions_repro]
...
print(independance(2895, 2944))
print(datesClesFusion(2944, 2895, 2895))
#%%
    ...
    for i in range(n_places):
        ...
        if type_p == 'point_monitoring':
            ...
            if len(id_measurepoints) == 1:
                ...
                # dates_cles = datesCles(exposureconditions)
            ...
            # S'il y a plus de 3 points de mesure à un endroit (place) --> gérer comme si les points étaient indépendants
                ...
                for id_mp in id_measurepoints:
                    natures = QueryScript(script='SELECT nature FROM pack WHERE measurepoint_id=' + str(id_mp)).execute()
            ...
            # sinon voir les points comme indépendants
                ...
                if resultat:  # Si les points sont indépendants (c'est à dire qu'ils ont des types de biotests communs)
                    ...
                    dates_cles = datesClesFusion(id_mp_alim, id_mp_chimie, id_mp_repro)
