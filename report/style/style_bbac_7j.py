...
def add_style_bbac_7j(bbac_dataframe, filename, folder_PATH):
    ...
    elements_metal = QueryScript(f" SELECT sandre, parameter, 7j_threshold, 7j_graduate_25, 7j_graduate_50, 7j_graduate_75   FROM {env.DATABASE_TREATED}.r3 WHERE familly='Métaux' AND 7j_threshold IS NOT NULL").execute()
    elements_organic = QueryScript(f" SELECT sandre, parameter, 7j_threshold, 7j_graduate_25, 7j_graduate_50, 7j_graduate_75   FROM {env.DATABASE_TREATED}.r3 WHERE familly!='Métaux' AND 7j_threshold IS NOT NULL").execute()
