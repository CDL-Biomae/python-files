...
def add_style_bbac_21j(bbac_dataframe, filename, folder_PATH):
    ...
    elements_metal = QueryScript(f" SELECT sandre, parameter, 21j_threshold, 21j_graduate_25, 21j_graduate_50, 21j_graduate_75   FROM {env.DATABASE_TREATED}.r3 WHERE familly='Métaux' AND 21j_threshold IS NOT NULL").execute()
    elements_organic = QueryScript(f" SELECT sandre, parameter, 21j_threshold, 21j_graduate_25, 21j_graduate_50, 21j_graduate_75   FROM {env.DATABASE_TREATED}.r3 WHERE familly!='Métaux' AND 21j_threshold IS NOT NULL").execute()
