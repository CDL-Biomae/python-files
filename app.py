...
def add_campaign():
    is_existing = QueryScript(f"SELECT * FROM {env.DATABASE_RAW}.campaign WHERE reference='{campaign_input.get()}'").execute()
    if len(is_existing):
        if len(campaign_list):
            campaign_input_text.set(campaign_input_text.get() + '\n')   
        campaign_input_text.set(campaign_input_text.get() + campaign_input.get())
...
campaign_button = tk.Button(master=frame_campaign,text="Ajouter", command=add_campaign)
campaign_button.grid(row=0, column=2)
campaign_input_text = tk.StringVar()
tk.Label(master=frame_campaign, text='Campagne(s) choisie(s) :').grid(row=1, column=0)
tk.Label(master=frame_campaign, textvariable=campaign_input_text).grid(row=1, column=1)
...
tk.Checkbutton(frame_choice, text='Rapport excel', variable=excel_wanted).pack()
word_wanted =  tk.IntVar(0)
tk.Checkbutton(frame_choice, text='Rapport word', variable=word_wanted).pack()
launch_button = tk.Button(master=frame_choice,text="Lancer", command=main_button)
launch_button.pack()
frame_choice.pack(expand='YES')

frame_folder = tk.Frame(master=window)
output_folder_path = tk.StringVar()
output_folder_button = tk.Button(master=frame_folder,text="Choisir une destination ...", command=output_browse_button)
output_folder_button.pack()
output_label = tk.Label(master=frame_folder,textvariable=output_folder_path).pack()
input_folder_path = tk.StringVar()
input_folder_button = tk.Button(master=frame_folder,text="Choisir une source des photos ...", command=input_browse_button)
input_folder_button.pack()
input_label = tk.Label(master=frame_folder,textvariable=input_folder_path).pack()
