...
def fill_temperature_repro(cas, temperatures):
    ...
    if cas == 2:
        ...
        QueryScript(f"DELETE FROM {env.DATABASE_TREATED}.temperature_repro WHERE version = {env.CHOSEN_VERSION()} and measurepoint_id in {tuple(need_update) if len(need_update)>1 else '('+(str(need_update[0]) if len(need_update) else '0')+')'};").execute(admin=True)
    ...
    liste_id = QueryScript(
        f" SELECT measurepoint_id FROM {env.DATABASE_TREATED}.average_temperature WHERE version=  {env.CHOSEN_VERSION()}").execute()

    constantes = QueryScript(
        f" SELECT name,value FROM {env.DATABASE_TREATED}.r2_constant WHERE nature='Temperature repro' and version=  {env.CHOSEN_VERSION()}").execute()
