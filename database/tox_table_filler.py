...
def get_dict_pack_fusion(campaign=None):
    if campaign:
        output = QueryScript(
            f"SELECT DISTINCT pack.id, key_dates.measurepoint_fusion_id, pack.nature FROM {env.DATABASE_RAW}.pack JOIN {env.DATABASE_TREATED}.key_dates ON key_dates.measurepoint_id=pack.measurepoint_id JOIN {env.DATABASE_RAW}.measurepoint ON measurepoint.id=pack.measurepoint_id WHERE measurepoint.reference LIKE '{campaign}%';"
        ).execute()
    else:    
        output = QueryScript(
            f"  SELECT DISTINCT pack.id, key_dates.measurepoint_fusion_id, pack.nature FROM {env.DATABASE_RAW}.pack JOIN {env.DATABASE_TREATED}.key_dates ON key_dates.measurepoint_id=pack.measurepoint_id;"
        ).execute()
